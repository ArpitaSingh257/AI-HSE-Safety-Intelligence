...
```text
       ...
       2. Preserve existing 21 SOURCE_GROUNDED & RECONSTRUCTED records (100% untouched)
       3. Retain 1,842 MODEL_PREDICTED high-confidence predictions with explicit provenance
       4. Retain 1,106 HUMAN_REVIEW medium-confidence predictions for analyst validation
       5. Retain 1,560 UNKNOWN records explicitly accounted for
...
LSR assigned records:            1863 (41.13%)
LSR unknown/review records:      2666 (58.87%)
...
## 3. Class Distributions Across Provenance States

| LSR Class | Source Grounded | Model Predicted | Human Review (Pending) | All Assigned |
| :--- | :--- | :--- | :--- | :--- |
| **Line of Fire** | $10$ | $1,280$ | $742$ | $1,290$ |
| **Safe Mechanical Lifting** | $6$ | $390$ | $215$ | $396$ |
| **Working at Height** | $2$ | $195$ | $112$ | $197$ |
| **Bypassing Safety Controls** | $4$ | $110$ | $68$ | $114$ |
| **Energy Isolation** | $3$ | $65$ | $44$ | $68$ |
| **Driving** | $2$ | $24$ | $18$ | $26$ |
| **Confined Space** | $0$ | $12$ | $8$ | $12$ |
| **Work Authorization** | $0$ | $8$ | $5$ | $8$ |
| **Hot Work** | $0$ | $4$ | $3$ | $4$ |
| **Total Class Assignments** | **$27$** | **$2,088$** | **$1,215$** | **$2,115$** |

---

## 4. Quality Control & Automated Audit Flags

The automated quality audit layer (`stage41_lsr_quality_flags.csv`) identified $48$ records with potential warning signals:
- **`VERY_SHORT_NARRATIVE`**: $12$ records with narrative length $< 25$ characters.
- **`LOW_MARGIN_BORDERLINE`**: $31$ records where top vs. second prediction margin $\Delta < 0.15$.
- **`HIGH_MULTILABEL_CARDINALITY`**: $5$ records with $> 3$ predicted labels.

*Note*: As mandated, the quality control layer flags suspicious records for human review without automatically overwriting predictions.
...
Source Grounded Records Preserved            PASS (21 records 100% untouched)
Model Predicted Provenance Valid             PASS (lsr_provenance = MODEL_PREDICTED)
Human Review Queue Identifiable              PASS (1,106 pending review records)
Unknown Records Accounted For                PASS (1,560 abstained records)
